...
yearFilteredData = FILTER weekExtractYear BY GetWeekYear(val_0) == 2010;
...
filterQuarterData = FILTER quarterData by symbol matches '$symbol';
quarterGrpData = GROUP filterQuarterData by Quarter;
quarterAvgData = FOREACH quarterGrpData GENERATE group as quarter,AVG(filterQuarterData.open) as avgOpen,AVG(filterQuarterData.high)as avgHigh,AVG(filterQuarterData.low) as avgLow,AVG(filterQuarterData.close) as avgClose,AVG( filterQuarterData.volume) as avgVolume,AVG(filterQuarterData.adjClose) as avgAdjClose;
STORE quarterAvgData into '$output' using PigStorage(',');